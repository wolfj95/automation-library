...
export default function EditAutomationPage() {
  ...
  useEffect(() => {
    async function loadAutomation() {
      setLoading(true);
      const automation = await DataService.getAutomationById(id);
      if (automation) {
        setFormData({
          title: automation.title,
          description: automation.description,
          studentName: automation.studentName,
          setupInstructions: automation.setupInstructions,
          installationCode: automation.installationCode || '',
          tags: automation.tags.join(', ')
        });
        setLinks(automation.links.length > 0 ? automation.links : [{ title: '', url: '' }]);
        setImages(automation.images.length > 0 ? automation.images : ['']);
      }
      setLoading(false);
    }
    loadAutomation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await DataService.updateAutomation(id, {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white shadow-sm border-b">
        <div className="container mx-auto px-4 py-6">
          <Link href={`/automation/${id}`} className="text-indigo-600 hover:text-indigo-800 font-medium">
            ← Back to Automation
          </Link>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="bg-white rounded-lg shadow-md p-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">Edit Automation</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-6 border-t border-gray-200 flex gap-4">
              ...
              <Link
                href={`/automation/${id}`}
                className="flex-1 py-3 px-6 rounded-lg font-medium text-gray-700 bg-gray-200 hover:bg-gray-300 transition-colors text-center"
              >
                Cancel
              </Link>
            </div>
